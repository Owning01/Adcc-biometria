...
const PRIORITY_IDS = [35631, 35624, 35617, 35610, 35730, 35740]; // Mas IDs para test
...
function parseAdccDate(dateStr) {
...
function isMatchInScope(match) {
    if (!match.dia) return true; // If no date, let it pass to check detail
    const normalizedDate = parseAdccDate(match.dia);
    if (!normalizedDate) return true; // Can't parse? Check detail
    // Broad check: allow everything from 2026 onwards for the list phase
    return normalizedDate >= '2026-01-01';
}
    ...
    for (let i = 0; i < scopedMatches.length; i += BATCH_SIZE) {
        ...
        await Promise.all(batch.map(async (match) => {
            ...
            try {
                ...
                const normalizedRealDate = parseAdccDate(detail.partido?.dia) || parseAdccDate(match.dia) || '';
                const isPriority = prioritySet.has(match.id);

                if (normalizedRealDate >= MIN_DATE || isPriority) {
