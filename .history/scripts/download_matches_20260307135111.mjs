...
async function main() {
    ...
    // Paso 2: Filtrar SOLO IDs prioritarios para verificación rápida
    const prioritySet = new Set(PRIORITY_IDS);
    const scopedMatches = allMatches.filter(m => prioritySet.has(m.id));

    console.log(`✅ Partidos en scope (SOLO PRIORITARIOS): ${scopedMatches.length}`);
