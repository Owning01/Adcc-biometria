...
export default function BatchPhotoProcessor() {
    const {
        getUrl, setGetUrl, players, status, progress, logs, errorMsg,
        currentPlayer, currentStep,
        handleLoadList, startProcessing, pauseProcessing, resetProcessor
    } = useBatchProcessor();
                ...
                <div style={{ display: 'flex', gap: '16px', marginBottom: '24px' }}>
                    {status === 'processing' ? (
                        ...
                        <button
                            ...
                            disabled={players.length === 0 || status === 'finished'}
                        >
                            <Play size={20} /> {status === 'paused' ? 'Continuar' : 'Iniciar Proceso'}
                        </button>
                    )}
                    <button onClick={resetProcessor} style={btnStyle('var(--danger-color)')} disabled={status === 'idle' || status === 'loading_list'}>
                        Resetear
                    </button>
                </div>
