...
export const MatchBatchProcessorProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const selectMatch = async (matchId: number) => {
        try {
            ...
            // Combinar planteles y filtrar los que no tienen biometría cargada (face_api: null)
            const allPlayers = [
            ...
            const players = allPlayers.filter(p => p.face_api !== null && p.face_api !== undefined);

            if (allPlayers.length > players.length) {
                log(`⚠️ ${allPlayers.length - players.length} jugadores ignorados por no tener face_api cargado.`);
            }
