...
export const BatchProcessorProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const syncAllPlayers = async () => {
        ...
        setStatus('syncing');
        log('☁️ Iniciando Sincronización con API ADCC (POST)...');

        // Aquí se podría implementar una lógica para sincronizar todos los que fueron exitosos
        // Por ahora, solo marcamos el inicio, ya que el usuario pidió sacarlo del proceso principal.
        // En una implementación real, buscaríamos en Firestore o usaríamos los resultados en memoria.

        setTimeout(() => {
            log('✅ Sincronización masiva finalizada (Simulado por ahora)');
            setStatus('finished');
        }, 2000);
    };
