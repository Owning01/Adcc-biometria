                ...
                await Promise.all(chunk.map(async (matchId) => {
                    try {
                        ...
                        const procPlayer = (p: any, tName: string) => {
                            // Prioridad: jugador_id (indicado por el usuario), luego fallback a id o jleid si jugador_id falta
                            const pid = p.jugador_id || p.id || p.jleid;
                            if (pid) {
    ...
    const processBatch = useCallback(async () => {
        ...
        for (let i = currentProgress.processed; i < playersToProcess.length; i++) {
            ...
            try {
                ...
                }, { syncWithApi: false, forceUpdate: true }); // ACTUALIZAR SIEMPRE
