...
export const BatchProcessorProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
                ...
                await Promise.all(chunk.map(async (matchId) => {
                    try {
                        ...
                        const procPlayer = (p: any, tName: string) => {
                            // El cruce de datos real es jugador_id (de /partidos) con el id (de /jugadores)
                            const pid = p.jugador_id;
                            if (pid && validPlayerIds.has(Number(pid))) {
                                newMap[Number(pid)] = { team: tName, category: categoria };
                            }
                        };
        ...
        for (let i = currentProgress.processed; i < playersToProcess.length; i++) {
            ...
            try {
                ...
                const result: RegistrationResult = await playerRegistrationService.registerPlayer({
                    id: player.id || player.jleid, // Priorizamos id de la lista global
                    dni: player.dni,
                    nombre: player.nombre || player.nombre_jugador,
                    apellido: player.apellido || player.apellido_jugador,
    ...
    return (
        <BatchProcessorContext.Provider value={{
            ...
            playerTeamMap, isMappingLoaded, loadTeamMapping
        }}>
            {children}
