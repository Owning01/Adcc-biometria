...
interface BatchProcessorContextType {
    ...
    handleLoadListSubset: (limit?: number) => Promise<void>;
    ...
    downloadImages: (players?: any[]) => Promise<void>;
}
...
export const BatchProcessorProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
        ...
        try {
            ...
            const normalized = filtered.map(p => {
                ...
                return {
                    ...
                    nombre,
                    apellido,
                    processed_foto: getAdccImageUrl(p.foto || p.imagen || '')
                };
            });
    ...
    const handleLoadListSubset = async (limit: number = 50) => {
        try {
            setStatus('loading_list');
            setErrorMsg('');
            setSuccessfulPlayers([]);
            ...
            log(`Cargando lote de prueba (máx ${limit} jugadores)...`);

            while (currentPageUrl && allPlayers.length < limit) {
                pageCount++;
                const actualUrl: string = currentPageUrl.startsWith('http')
                    ? currentPageUrl.replace('https://adccanning.com.ar', '/api-adcc')
                    : currentPageUrl;
                ...
                log(`Página ${pageCount} cargada: ${allPlayers.length} acumulados.`);
            ...
            const normalized = filtered.map(p => {
                ...
                return { ...p, nombre, apellido, processed_foto: getAdccImageUrl(p.foto || p.imagen || '') };
            });

            setPlayers(normalized);
            setProgress({ processed: 0, total: normalized.length, success: 0, failed: 0 });
            setStatus('ready');
            log(`Lote de prueba de ${normalized.length} jugadores cargado correctamente.`);
        } catch (err: any) {
            setStatus('error');
            setErrorMsg(err.message || 'Error cargando lote de prueba');
            log(`❌ Error lote prueba: ${err.message}`);
        }
    };
    ...
    const processBatch = useCallback(async () => {
        ...
        for (let i = currentProgress.processed; i < playersToProcess.length; i++) {
            ...
            try {
                ...
                // Validación CRÍTICA: No procesar si faltan datos esenciales
                if (!player.dni && !player.id) {
                    currentProgress.failed++;
                    log(`⚠️ SALTEADO: ${player.nombre || 'Sin nombre'} - Falta DNI e ID`);
                    continue;
                }
    ...
    const downloadImages = async (customPlayers?: any[]) => {
        const targetPlayers = customPlayers || players;

        if (targetPlayers.length === 0) {
            log('⚠️ No hay jugadores para descargar.');
            return;
        }

        log(`📦 Generando ZIP con ${targetPlayers.length} imágenes...`);
        const zip = new JSZip();
        let downloaded = 0;
        let errors = 0;

        for (const player of targetPlayers) {
            const fotoUrl = player.processed_foto || player.photo || player.imagen || player.imagen_url;
            if (!fotoUrl) continue;

            try {
                // Proxy para evitar CORS en desarrollo
                const proxiedUrl = fotoUrl.includes('adccanning.com.ar')
                    ? fotoUrl.replace('https://adccanning.com.ar', '/api-adcc')
                    : fotoUrl;
                ...
                const name = player.nombre || (player.name?.split(' ')[0]) || 'Jugador';
                const surname = player.apellido || (player.name?.split(' ').slice(1).join(' ')) || '';
                const fileName = `${name}_${surname}_${player.id || player.jleid}.jpg`
                    .replace(/[^a-z0-9.]/gi, '_');
                ...
                // Log progresivo
                if (downloaded % 10 === 0) {
                    log(`Agregadas ${downloaded}/${targetPlayers.length} imágenes al ZIP...`);
                    // Pequeño respiro para el main thread
                    await new Promise(r => setTimeout(r, 10));
                }
            } catch (err) {
                console.error(`Error procesando imagen de ${player.id}:`, err);
                errors++;
            }
        }

        if (downloaded > 0) {
            log('Generating ZIP file, please wait...');
            const content = await zip.generateAsync({ type: 'blob' });
            saveAs(content, `fotos_adcc_${new Date().toISOString().split('T')[0]}.zip`);
            log(`✅ ZIP descargado con ${downloaded} imágenes.`);
            if (errors > 0) log(`⚠️ Hubo ${errors} errores al obtener algunas imágenes.`);
        } else {
            log('❌ No se pudo descargar ninguna imagen.');
        }
    };
    ...
    return (
        <BatchProcessorContext.Provider value={{
            ...
            setGetUrl, handleLoadList, handleLoadListSubset, startProcessing, pauseProcessing,
