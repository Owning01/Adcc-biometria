...
export const BatchProcessorProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const downloadImages = async (customPlayers?: any[]) => {
        ...
        log(`Iniciando descarga de ${targetPlayers.length} imágenes...`);
        let downloaded = 0;

        for (const player of targetPlayers) {
            const fotoUrl = player.processed_foto || player.photo;
            if (!fotoUrl) continue;
            ...
            try {
                ...
                const fileName = `${name}_${surname}_${player.id}.jpg`
                    .replace(/[^a-z0-9.]/gi, '_');
                ...
                if (downloaded % 5 === 0) {
                    ...
                    log(`Descargadas ${downloaded}/${targetPlayers.length} imágenes...`);
                }
            } catch (err) {
