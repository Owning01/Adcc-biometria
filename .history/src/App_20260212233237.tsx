...
function App() {
  ...
  useEffect(() => {
    ...
    const runEffect = async () => {
      ...
      console.log(`[FaceLogin] Usuarios cargados: ${currentUsers.length}`);

      if (currentUsers.length > 0) {
        const newMatcher = createMatcher(currentUsers);
        matcherRef.current = newMatcher;
        console.log(`[FaceLogin] Matcher listo con ${currentUsers.length} registros`);
        setStatusText('Escaneando...');
      } else {
        console.warn("[FaceLogin] No se pudieron cargar usuarios.");
    ...
    const performLogin = async (video: HTMLVideoElement) => {
      ...
      try {
        ...
        if (data && matcherRef.current) {
          ...
          console.warn("[FaceLogin] Matcher no listo al intentar login");
