    ...
    try {
        ...
        while (currentPage <= lastPage) {
            ...
            try {
                ...
                if (!isPublicUser) console.error(`[Sync] Error fatal procesando página ${currentPage}:`, pageError);
                // Si la primera página falla, probablemente las demás también, pero intentaremos seguir si no es syncAll
                if (options.syncAll && currentPage === 1) throw pageError;
            }
        ...
        if (!isPublicUser) console.log('[Sync] Sincronización finalizada.');
        return true;
    } catch (error) {
        if (!isPublicUser) {
            console.error('[Sync] Error crítico en la sincronización global:', error);
        }
        return false;
    }
};
