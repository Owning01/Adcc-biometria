...
export const playerRegistrationService = {
    ...
    /**
     * Elimina todos los jugadores registrados de un equipo específico.
     */
    async deleteTeamData(teamName: string): Promise<{ success: boolean; deletedCount: number; error?: string }> {
        try {
            const { query, where, getDocs, collection, writeBatch } = await import('firebase/firestore');
            const collectionName = 'users';
            const q = query(collection(db, collectionName), where('team', '==', teamName));
            const querySnap = await getDocs(q);
            ...
            await batch.commit();
            return { success: true, deletedCount: querySnap.size };
        } catch (error: any) {
