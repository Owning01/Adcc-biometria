...
import { collection, addDoc, getDocs, onSnapshot, deleteDoc, doc, updateDoc, query, where, orderBy, serverTimestamp, setDoc } from 'firebase/firestore';
...
export const subscribeToTournaments = (callback: (tournaments: any[]) => void) => {
    ...
    return onSnapshot(q, (querySnapshot) => {
        ...
        // Ordenar en memoria por createdAt (o fecha) si existe
...
/**
 * Guarda o actualiza un torneo con un ID específico (útil para importaciones).
 * 
 * @param {string} id - El ID personalizado.
 * @param {Object} tournament - Datos del torneo.
 */
export const saveTournamentWithId = async (id: string, tournament: any) => {
    try {
...
export const getTournament = async (id: string): Promise<Tournament | null> => {
    try {
        ...
        const docSnap = await getDoc(doc(db, TOURNAMENTS_COLLECTION, id));
        if (docSnap.exists()) return { ...docSnap.data() as any, id: docSnap.id };
        return null;
    } catch (error) {
        return null;
    }
...
export const saveMatch = async (match: any) => {
    try {
        const docRef = await addDoc(collection(db, MATCHES_COLLECTION), {
            ...match,
            status: match.status || 'scheduled', // Estados: scheduled, live, halftime, finished
...
/**
 * Guarda o actualiza un partido usando un ID específico (idempotencia).
 * Útil para importaciones masivas.
 * 
 * @param {string} customId - ID del documento (ej: match_123)
 * @param {Object} match - Datos del partido
 */
export const saveMatchWithId = async (customId: string, match: any) => {
    try {
        const docRef = doc(db, MATCHES_COLLECTION, customId);
        await setDoc(docRef, {
            ...match,
...
export const subscribeToMatchEvents = (matchId: string, callback: (events: any[]) => void) => {
    ...
    return onSnapshot(q, (querySnapshot) => {
        const events = querySnapshot.docs.map(doc => ({
            ...
            timestamp: doc.data().serverTimestamp?.toDate ? doc.data().serverTimestamp.toDate() : (doc.data().localTimestamp ? new Date(doc.data().localTimestamp) : new Date())
        }));

        // Ordenar en memoria por timestamp
        events.sort((a: any, b: any) => a.timestamp.getTime() - b.timestamp.getTime());
