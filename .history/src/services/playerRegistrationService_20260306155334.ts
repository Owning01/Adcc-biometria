...
export const playerRegistrationService = {
    ...
    async registerPlayer(player: {
        ...
        team?: string;
        category?: string;
    }, options?: { syncWithApi?: boolean; forceUpdate?: boolean }): Promise<RegistrationResult> {
        try {
            const collectionName = 'users';
            const finalDocId = String(player.jleid || player.id || player.dni || Date.now());
            ...
            if (!data) {
                return { success: false, step: 'face_detection', error: 'No se detectó rostro con alta precisión en la imagen proporcionada' };
            }
            ...
            const playerDoc = {
                id: player.jleid ?? player.id ?? player.dni ?? '', // Evitar undefined para Firestore
                dni: player.dni ?? '',
                ...
                team: player.team || existingTeam || 'Sin Equipo',
                category: player.category || existingCategory || 'Faltante',
            };
