...
const Equipos = ({ userRole }: { userRole: string }) => {
    ...
    const handleFullSync = async () => {
        if (syncCooldown) {
            alert("Por favor, espera unos segundos antes de volver a sincronizar.");
            return;
        }
        ...
        try {
            ...
            if (err?.message?.includes('429')) {
                alert("Servidor saturado (Error 429). Por favor, intenta de nuevo en un minuto.");
            } else {
                alert("Error en sincronización");
            }
        } finally {
            setSyncingAll(false);
            setTimeout(() => setSyncCooldown(false), 5000); // 5 seconds cooldown
        }
    };
