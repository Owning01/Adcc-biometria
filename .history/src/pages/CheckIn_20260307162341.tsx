...
const CheckIn = () => {
    ...
    const [pendingNumberUser, setPendingNumberUser] = useState<{ user: any, match: any } | null>(null);
    ...
    const fetchUsers = async () => {
        try {
            ...
            console.error("Error fetching users for checkin:", err);
        }
    };
                ...
                if (faceData && faceData.descriptor && faceData.detection) {
                    ...
                    if (match.label !== 'unknown') {
                        ...
                        if (user) {
                            ...
                            if (userMatch) {
                                // AUTO-MARCAR COMO PRESENTE INMEDIATAMENTE: jugo = 1
                                const teamKey = userMatch.teamA.name === user.team ? 'playersA' : 'playersB';
                                const players = [...(userMatch[teamKey] || [])];
                                const playerIndex = players.findIndex((p: any) => p.dni === user.dni || p.id === user.id);

                                if (playerIndex >= 0) {
                                    players[playerIndex] = { ...players[playerIndex], jugo: 1 };
                                    if (!players[playerIndex].status) players[playerIndex].status = 'titular';
                                } else {
                                    players.push({ ...user, jugo: 1, status: 'titular' });
                                }
                                updateMatch(userMatch.id, { [teamKey]: players }).catch(console.error);
    ...
    return (
        <div className="container animate-fade-in" style={{ height: 'calc(100vh - 180px)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div className="webcam-wrapper" style={{ width: '100%', maxWidth: '500px', aspectRatio: '1/1', maxHeight: '70vh', position: 'relative' }}>
                <Webcam
                    audio={false}
                    ref={webcamRef}
                    width="100%"
            ...
            <button
                ...
                className="glass-button"
                style={{ marginTop: '25px', padding: '12px 24px', fontSize: '0.85rem', background: 'rgba(59, 130, 246, 0.15)', borderColor: 'var(--primary)', letterSpacing: '1px', fontWeight: '800' }}
            >
                <Search size={18} style={{ marginRight: '10px' }} /> ¿NO ESTÁS REGISTRADO? BUSCAR EN ADCC
            </button>
