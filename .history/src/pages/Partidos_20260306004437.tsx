...
const Partidos = ({ userRole }: { userRole: string }) => {
    ...
    useEffect(() => {
        // Disparar sincronización automática en segundo plano (2 páginas rápidas)
        syncADCCData({ pages: 2, userRole }).catch(err => {
            ...
            {userRole === 'public' && (
                <div className="show-mobile-flex" style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <img src={AppLogo} alt="Logo" style={{ height: '30px' }} />
                        <span style={{ fontWeight: '800', fontSize: '1rem', letterSpacing: '-0.5px', color: '#fff' }}>ADCC</span>
                    </div>
                    <m.button
                        whileTap={{ scale: 0.95 }}
                ...
                <div className="header-controls" style={{ display: 'flex', flexWrap: 'wrap', gap: '15px', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', gap: '10px', flex: '1 1 auto' }}>
                        ...
                        >
                            ...
                            {Array.from(new Set(matches.filter(m => m.tournamentName).map(m => m.tournamentName))).sort().map(tName => (
                ...
                <div className="match-list-container">
                    {matches.length === 0 ? (
                        ...
                        matches.filter(m => {
                            ...
                            const matchesTournament = selectedTournament === 'all' || m.tournamentName === selectedTournament;
