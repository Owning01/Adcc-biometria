...
const Equipos: React.FC<EquiposProps> = ({ userRole }) => {
    ...
    const handleFullSync = async () => {
        ...
        setSyncingAll(true);
        try {
            await syncADCCData({ syncAll: true, userRole });
            alert("Sincronización completa finalizada.");
        } catch (error) {
            if (isAdmin) console.error("Sync error:", error);
                    ...
                    {(!isUsuario && !isPublic) && (
                        <div className="glass-panel" style={{ padding: '20px', display: 'flex', flexWrap: 'wrap', gap: '15px', background: 'rgba(212, 175, 55, 0.05)', borderColor: 'rgba(212, 175, 55, 0.1)' }}>
                            ...
                            <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                                ...
                                <button onClick={() => setShowGlobalSearch(true)} className="glass-button" style={{ padding: '10px 20px', fontSize: '0.75rem', background: 'rgba(59, 130, 246, 0.2)', borderColor: '#60a5fa' }}><Search size={16} /> BÚSQUEDA GLOBAL ADCC</button>
                            </div>
                        </div>
                    )}
