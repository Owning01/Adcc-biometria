                        ...
                        <div className="glass-panel" style={{ maxWidth: '400px', width: '100%', padding: '30px', position: 'relative', border: '1px solid rgba(255,255,255,0.1)' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                                ...
                                <button
                                    ...
                                    className="glass-button w-full"
                                    style={{ padding: '15px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', marginTop: '10px' }}
                                >
                                    <Repeat2 size={20} />
                                    <span style={{ fontWeight: 'bold' }}>{selectedPlayer.player.status === 'titular' ? 'MOVER A BANCA' : 'MOVER A TITULAR'}</span>
                                </button>
                            </div>
                        </div>
                    </div>
                        ...
                        <div key={idx}
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginLeft: 'auto' }}>
                                {parseInt(p.yellowCards) > 0 && (
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '2px', background: 'rgba(251, 191, 36, 0.1)', padding: '2px 6px', borderRadius: '4px' }}>
                                        <div style={{ width: '8px', height: '12px', background: '#fbbf24', borderRadius: '1.5px' }}></div>
                                        <span style={{ fontSize: '0.8rem', color: '#fbbf24', fontWeight: 'bold' }}>{p.yellowCards}</span>
                                    </div>
                                )}
                                {p.redCard && (
                                    <div style={{ width: '8px', height: '12px', background: '#ef4444', borderRadius: '1.5px' }}></div>
                                )}
                                {parseInt(p.goals) > 0 && (
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px', background: 'rgba(16, 185, 129, 0.1)', padding: '2px 8px', borderRadius: '4px' }}>
                                        <Target size={12} color="#10b981" />
                                        <span style={{ fontSize: '0.8rem', color: '#10b981', fontWeight: 'bold' }}>{p.goals}</span>
                                    </div>
                                )}
                            </div>
}
