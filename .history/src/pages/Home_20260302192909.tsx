...
const Home = ({ userRole }: { userRole?: string }) => {
    ...
    useEffect(() => {
        // Disparar sincronización automática en segundo plano (1 página rápida)
        syncADCCData({ pages: 1 }).catch(err => console.error("Auto-sync failed:", err));
                                    ...
                                    <div key={team} onClick={() => setSelectedTeam(team)} className="team-card">
                                        <div className="hexagon-container">
                                            <div className="team-card-content">
                                                {teamData?.logoUrl ? (
                                                    <img src={teamData.logoUrl} alt={team} className="team-logo-img" style={{ width: '45px', height: '45px', objectFit: 'contain', marginBottom: '8px' }} onError={(e) => (e.currentTarget.src = 'https://placehold.co/100x100?text=Team')} />
                                                ) : teamData?.adccLogoUrl ? (
                                                    <img src={getAdccImageUrl(teamData.adccLogoUrl)} alt={team} className="team-logo-img" style={{ width: '45px', height: '45px', objectFit: 'contain', marginBottom: '8px' }} onError={(e) => (e.currentTarget.src = 'https://placehold.co/100x100?text=Team')} />
                                                ) : (
