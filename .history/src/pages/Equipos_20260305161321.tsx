...
const Equipos = ({ userRole }: { userRole: string }) => {
    ...
    // Modales de Equipos/Categorías
    const [showTeamModal, setShowTeamModal] = useState(false);
    const [showCategoryModal, setShowCategoryModal] = useState({ open: false, team: '' });
    const [renameModal, setRenameModal] = useState({ open: false, type: 'team', oldName: '', team: '', newValue: '' });
    const [modalInput, setModalInput] = useState('');
    const [teamSearch, setTeamSearch] = useState('');
    const [expandedCategory, setExpandedCategory] = useState<{ team: string; category: string } | null>(null);
    ...
    const [quickRegisterData, setQuickRegisterData] = useState({ team: '', category: '' });
    ...
    useEffect(() => {
        ...
        const unsubTeams = subscribeToTeams((data) => {
            ...
            data.forEach(t => {
                if (t.categories) cats[t.name] = t.categories;
            });
    ...
    const handleOpenQuickRegister = (team: string, category: string) => {
        setQuickRegisterData({ team, category });
