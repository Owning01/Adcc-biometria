/**
 * @file AltaLocal.tsx
 * @description PANTALLA DE ALTA/RECONOCIMIENTO (KIOSK MODE)
 * Interfaz optimizada para tablet/kiosco que permite:
 * 1. Detección facial rápida (MediaPipe).
 * 2. Verificación biométrica profunda (Face-API).
 * 3. Feedback visual y sonoro inmediato.
 */
// ============================================================================
// 1. IMPORTS
// ============================================================================
            ...
            try {
                ...
                if (result.success) {
                    ...
                    unsubscribe = subscribeToUsers((data) => {
                        if (!data) return;
                        setUsers(data);
                        // Solo regenerar el matcher si la cantidad de usuarios cambió
                        // Esto evita lag innecesario en cada snapshot de Firestore
                        setMatcher(prev => {
                            if (prev && prev.labeledDescriptors.length === data.length) return prev;
                            return createMatcher(data);
                        });
                    });
                } else {
    ...
    useEffect(() => {
        ...
        const performDeepRecognition = async (video) => {
            ...
            try {
                ...
                // Error en reconocimiento profundo
            } finally {
        ...
        if (!matchResult && !isModelsLoading) {
            interval = setInterval(detectFaceHybrid, 250); // Aumentado de 150ms a 250ms
        }
