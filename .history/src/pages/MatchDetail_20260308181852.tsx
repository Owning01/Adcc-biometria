...
const SquadColumn = ({ title, logoUrl, players, teamSide, onAdd, onSubstitution, onUpdate, onRemove, isReferee, userRole, onPlayerClick, onPhotoClick }: SquadColumnProps) => {
    ...
    return (
        <div className="glass-panel overflow-hidden border border-white/10 flex flex-col h-full bg-white/5 backdrop-blur-md">
            {/* Squad Header */}
            <div className="p-4 border-bottom border-white/10 flex items-center justify-between bg-gradient-to-r from-primary/10 to-transparent">
            ...
            <div className="p-2 flex flex-col gap-2 max-h-[600px] overflow-y-auto custom-scrollbar">
                {players.length === 0 ? (
                    ...
                    players.map((p: any, idx: number) => (
                        ...
                        >
                            {/* Player Number */}
                            <div className="w-10 h-10 flex-shrink-0 bg-dark-soft border border-white/10 rounded-lg flex items-center justify-center text-lg font-black text-primary shadow-inner">
                                <input
                                    type="number"
                                    value={p.number}
                            ...
                            <div className="flex-1 min-w-0">
                                <div className={`font-bold truncate text-[0.95rem] ${p.status === 'expulsado' ? 'text-red-500' : 'text-white'} ${p.status === 'suplente' ? 'opacity-60' : ''}`}>
                                    {p.name}
                                    {p.isDisabled && <span className="ml-2 text-red-500 animate-pulse">⚠️</span>}
                                    {p.status === 'expulsado' && <span className="ml-2 px-1.5 py-0.5 bg-red-500 text-[0.6rem] text-white rounded font-black uppercase tracking-[1px]">ROJA</span>}
                                </div>
