    ...
    return (
        ...
        >
            <div className="match-header-info">
                <div className="flex-row-center" style={{ gap: 'var(--space-sm)' }}>
                    <Calendar size={14} className="text-primary" />
                    <span className="text-xs text-muted text-bold text-uppercase">{match.date}</span>
                </div>
                <div className="flex-row-center" style={{ gap: 'var(--space-sm)' }}>
                    <Clock size={14} className="text-primary" />
                    <span className="text-xs text-muted text-bold">{match.time} HS</span>
                </div>
            ...
            <div className="match-teams-container">
                <div className="match-team-display">
                    <div className="team-logo-container">
                        {teamALogo ? <img src={teamALogo} alt={match.teamA?.name} className="match-team-logo" /> : <Shield size={24} className="opacity-20" />}
                    </div>
                    <span className="match-team-name">{match.teamA?.name || 'Equipo A'}</span>
                </div>

                <div className="flex flex-col items-center gap-2">
                    <div className="match-score-pill">
                        <span className="match-score-number">{match.score?.a ?? 0}</span>
                        <span className="text-muted opacity-30 text-xs">-</span>
                        <span className="match-score-number">{match.score?.b ?? 0}</span>
                    </div>
                    {match.status === 'live' && <span className="text-[10px] text-red-500 font-bold uppercase tracking-widest animate-pulse">En Vivo</span>}
                </div>

                <div className="match-team-display">
                    <div className="team-logo-container">
                        {teamBLogo ? <img src={teamBLogo} alt={match.teamB?.name} className="match-team-logo" /> : <Shield size={24} className="opacity-20" />}
                    </div>
                    <span className="match-team-name">{match.teamB?.name || 'Equipo B'}</span>
                </div>
            </div>

            <div className="match-footer-status">
                <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-tighter ${isFinished ? 'bg-white/5 text-muted' : 'bg-primary/10 text-primary border border-primary/20'}`}>
                    {getStatusLabel(match.status)}
                    {(match.status === 'halftime' || match.status === 'finished') && (
                        <span className="ml-1 opacity-60">({formatMatchTime(match)})</span>
                    )}
                </span>
            </div>
        </m.div>
