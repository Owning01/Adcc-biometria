...
export default function MatchImporter() {
    ...
    const handleSpecificIds = () => {
        const ids = [35631, 35624, 35617, 35610];
        startSpecificProcessing(ids);
    };
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (event) => {
            try {
                ...
                if (json.matches && json.details) {
                    startLocalProcessing(json);
                } else {
                    alert('El formato del JSON no es válido. Debe contener "matches" y "details".');
                }
            } catch (err) {
                ...
                <div style={{ display: 'flex', gap: '12px' }}>
                    ...
                    {(status === 'idle' || status === 'ready' || status === 'paused' || status === 'finished') && (
                        <>
                            ...
                            >
                                <Zap size={18} /> Procesar IDs Críticos
                            </button>
                            <input
                                type="file"
                                ref={fileInputRef}
                            ...
                            >
                                <Upload size={18} /> Procesar JSON Local
                            </button>
                            <button
