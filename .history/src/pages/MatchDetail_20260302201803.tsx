...
import { Mic, MicOff, Trophy, Users, Activity, BarChart2, Plus, ArrowLeft, Calendar, Clock, User as UserIcon, Trash2, Minus, Target, Zap, Star, Repeat2, Square, Info, ShieldAlert, Award, TrendingUp, Save, X } from 'lucide-react';
...
const MatchDetail = ({ userRole }: { userRole: string }) => {
    ...
    const submitMatchReport = async () => {
        ...
        const formatPlayers = (players: any[]) => {
            return (players || []).map(p => {
                ...
                return {
                    ...
                    roja: !!p.redCard ? 1 : 0,
                    jugo: (p.status === 'titular' || p.status === 'suplente_entro' || !!p.redCard) ? 1 : 0
                };
            });
        };
        ...
        try {
            const response = await fetch('/api-adcc/api/cargaPlanillaPartido', {
                method: 'POST',
                headers: {
                    'Authorization': 'Bearer 9f4b7d2a5e3c1a8f6b0d9c4e7a3b8f1c2d6e0f9a4b3c2d1e0f7a6b5c4d3e2f1',
            ...
            if (response.ok) {
                ...
                // Error API
                alert("❌ Error al enviar la planilla. Verifique los datos e intente nuevamente.");
            }
        } catch (error) {
            // Error de conexión
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                    <div className="flex-center" style={{ gap: '15px' }}>
                        {(isAdminOrDev || isReferee) && (
                            <button
                                onClick={toggleVoice}
                                className={`voice-control-btn ${isVoiceActive ? 'active' : ''}`}
                                title={isVoiceActive ? "Desactivar Voz" : "Activar Árbitro por Voz"}
                            >
                        ...
                        {isAdminOrDev && (
                            <button
                                ...
                                style={{ background: 'rgba(52, 211, 153, 0.1)', color: '#34d399', fontSize: '0.8rem' }}
                            >
                                <Save size={16} /> CARGAR PLANILLA
                            </button>
                        )}
                    </div>
                ...
                <div style={{ position: 'absolute', top: '15px', right: '15px' }}>
                    {(isAdminOrDev || isReferee) ? (
                        <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                            {['scheduled', 'live', 'halftime', 'finished'].map(st => (
                                <button
                                    key={st}
                                    onClick={() => changeMatchStatus(st)}
                                    className={`status-chip ${match.status === st ? 'active' : ''}`}
                                >
                                    {st === 'live' && <Activity size={12} className="animate-pulse" />}
                                    {st.toUpperCase()}
                                </button>
                            ))}
                        </div>
                    ) : (
                        <div className="status-chip active" style={{ margin: '0 auto', width: 'fit-content' }}>
                ...
                <div className="scoreboard-grid">
                    ...
                    <div className="scoreboard-result">
                        <div className="flex-center" style={{ gap: '20px' }}>
                            {isAdminOrDev || isReferee ? (
                                <button onClick={() => handleScoreChange('a', 1)} className="score-control-btn">+</button>
                            ) : null}
                            ...
                            {isAdminOrDev || isReferee ? (
                                <button onClick={() => handleScoreChange('b', 1)} className="score-control-btn">+</button>
                            ) : null}
                        </div>
                        {match.status === 'live' && (
                    ...
                    <div className="glass-panel" style={{ maxWidth: '800px', width: '100%', maxHeight: '90vh', overflowY: 'auto', padding: '40px', border: '1px solid var(--primary)', position: 'relative' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '40px' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                                <div style={{ background: 'rgba(212, 175, 55, 0.1)', padding: '15px', borderRadius: '12px', border: '1px solid rgba(212, 175, 55, 0.2)' }}>
...
const SquadColumn = ({ title, logoUrl, players, teamType, onAdd, onSubstitution, onUpdate, onRemove, isReferee, userRole }: any) => {
