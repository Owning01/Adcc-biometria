z< import React, { useReducer, useState, useEffect, ChangeEvent } from 'react';
...
const DevTools: React.FC = () => {
    ...
    const loadADCCMatches = async () => {
        ...
        try {
            ...
            alert("Error al cargar partidos de ADCC");
        } finally {
    ...
    const handleSyncAllADCCPlayers = async () => {
        ...
        try {
            ...
            alert("Error durante la sincronización masiva: " + (error.message || String(error)));
        } finally {
            dispatch({ type: 'FINISH_IMPORT' });
        }
    };
            ...
            <div className="container animate-fade-in">
                ...
                <div style={{ display: 'flex', gap: '12px', marginBottom: '30px', justifyContent: 'center', flexWrap: 'wrap' }}>
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 px-6 py-3 rounded-xl transition-all ${mode === 'matches' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-white text-slate-600 hover:bg-slate-50'}`}
                    >
                    ...
                    <button
                        ...
                        className={`glass-button ${mode === 'adcc' ? '' : 'button-secondary'}`}
                        style={{ fontSize: '0.75rem', padding: '12px 24px' }}
                    >
                                            ...
                                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '15px' }}>
                                                <div style={{ flex: 1, textAlign: 'center' }}>
                                                    <div style={{ width: '60px', height: '60px', background: 'rgba(255,255,255,0.05)', borderRadius: '15px', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '8px', margin: '0 auto 10px' }}>
                                                        <img
                                                            src={match.local_escudo?.startsWith('http') ? match.local_escudo : `https://adccanning.com.ar/img/${match.local_escudo}`}
                                                            alt={match.local_nombre}
                                                            style={{ width: '100%', height: '100%', objectFit: 'contain' }}
                                                            onError={(e) => { (e.target as HTMLImageElement).src = 'https://adccanning.com.ar/img/logo-placeholder.png' }}
                                                        />
                                                    </div>
                                                ...
                                                <div style={{ flex: 1, textAlign: 'center' }}>
                                                    <div style={{ width: '60px', height: '60px', background: 'rgba(255,255,255,0.05)', borderRadius: '15px', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '8px', margin: '0 auto 10px' }}>
                                                        <img
                                                            src={match.visitante_escudo?.startsWith('http') ? match.visitante_escudo : `https://adccanning.com.ar/img/${match.visitante_escudo}`}
