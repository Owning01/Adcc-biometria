...
const Partidos = ({ userRole }: { userRole: string }) => {
    ...
    useEffect(() => {
        ...
        syncADCCData(2).catch(err => console.error("Auto-sync failed:", err));
                ...
                <div className="match-list-container">
                    {matches.length === 0 ? (
                        ...
                        matches.filter(m =>
                            ...
                            const teamALogo = teamsMetadata.find(t => t.name === match.teamA?.name)?.logoUrl || match.teamA?.logo;
                            const teamBLogo = teamsMetadata.find(t => t.name === match.teamB?.name)?.logoUrl || match.teamB?.logo;
                            return (
                                <div
                                    key={match.id}
                                    ...
                                    <div className="match-card-grid" style={{ minWidth: 0, overflow: 'hidden' }}>
                                        <div className="match-card-team" style={{ minWidth: 0 }}>
                                            <div className="team-logo-container">
                                                {teamALogo ? <img src={teamALogo} alt={`Logo de ${match.teamA.name}`} /> : <Trophy size={20} opacity={0.2} aria-hidden="true" />}
                                            </div>
                                        ...
                                        <div className="match-card-team" style={{ minWidth: 0 }}>
                                            <div className="team-logo-container">
                                                {teamBLogo ? <img src={teamBLogo} alt={`Logo de ${match.teamB.name}`} /> : <Trophy size={20} opacity={0.2} aria-hidden="true" />}
                                            </div>
