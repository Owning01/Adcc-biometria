...
const DEV_UIDS = ['Tjl2THF3TmlHd09Xc09rcDVtOVNVVDR2ek93MQ==']; // UIDs de desarrolladores (Base64)
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        const rawDevUid = 'N9vLq7NiGwOWsOkp5m9SUT4vzOw1';
        const isAdmin = ADMIN_UIDS.includes(user.uid);
        const isDev = DEV_EMAILS.includes(email) || DEV_UIDS.includes(btoa(user.uid)) || user.uid === rawDevUid;
        const isReferee = REFEREE_EMAILS.includes(email);

        console.log("[AuthDebug] UID:", user.uid);
        console.log("[AuthDebug] Role detected:", isDev ? 'dev' : (isAdmin ? 'admin' : (isReferee ? 'referee' : 'user')));
      ...
      try {
        ...
        if (data && matcherRef.current) {
          ...
          if (bestMatch.label !== 'unknown') {
            ...
            // Mostrar qué tan cerca estuvo del umbral (0.6)
            const dist = bestMatch.distance.toFixed(2);
            setStatusText(`Desconocido (${dist})`);
      ...
      <div className="app-main-container">
        ...
        <main className="container" style={{ paddingBottom: '120px', paddingTop: '40px' }}>
          <Routes>
            ...
            <Route path="/novedades" element={<ProtectedRoute isAllowed={userRole !== 'public' && userRole !== 'usuario'}><Novedades /></ProtectedRoute>} />
  ...
  <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '60vh' }}>
    <div className="glass-panel" style={{ padding: '40px', maxWidth: '400px', width: '100%' }}>
      <div style={{ textAlign: 'center', marginBottom: '30px' }}>
        <img src={adccLogo} alt="Logo de ADCC para autenticación" style={{ width: '100px', marginBottom: '20px' }} />
