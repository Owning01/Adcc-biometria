...
const REFEREE_UIDS = ['Fpu2WsnqOYQCAk59Ts2On6Mpjcl2']; // UID del Árbitro
// Emails ofuscados en Base64 para evitar exposición en el código fuente
const DEV_EMAILS_B64 = ['ZGV2QGFkbWluLmNvbQ==', 'YWRtaW5AYWRtaW4uY29t'];
...
function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        const isDev = DEV_EMAILS_B64.includes(emailB64) || DEV_UIDS.includes(btoa(user.uid));
        const isReferee = REFEREE_EMAILS.includes(email) || REFEREE_UIDS.includes(user.uid);
  ...
  useEffect(() => {
    ...
    const runEffect = async () => {
      ...
      }, 250); // Aumentado de 150ms a 250ms para reducir carga de CPU
    };
      ...
      try {
        ...
        if (data && matcherRef.current) {
          ...
          if (bestMatch.label !== 'unknown') {
            ...
            } catch (e) { /* Fallback si no es nativo */ }
      ...
      <div style={{
        ...
        <div style={{
          ...
          <img
            src={adccLogo}
            alt="ADCC"
            style={{ width: '80px', filter: 'brightness(1.1)' }}
          />
          <div style={{
            fontSize: '0.8rem',
            letterSpacing: '4px',
...
const Navigation: React.FC<NavigationProps> = ({ userRole, onLogout, theme, toggleTheme }) => {
  ...
  if (userRole === 'public') {
    return (
      <nav className="app-nav hide-mobile">
        <div className="nav-brand">
          <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: '10px', textDecoration: 'none' }}>
            <img src={adccLogo} alt="Logo ADCC" style={{ height: '35px', objectFit: 'contain' }} />
            <div style={{ fontWeight: '800', letterSpacing: '-1px', color: 'var(--primary)', fontSize: '1.2rem', marginLeft: '5px' }}>ADCC</div>
          </Link>
        </div>
  ...
  return (
    <>
      <nav className="app-nav">
        ...
        {/* Bottom Navigation for Mobile - Square & Docked */}
        <div className="show-mobile-flex bottom-nav-mobile docked-square">
          {userRole !== 'public' && (
