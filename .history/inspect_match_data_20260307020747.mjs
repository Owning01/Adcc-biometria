    ...
    for (const id of ids) {
        ...
        try {
            ...
            if (detail.equipo_local && detail.equipo_local.length > 0) {
                console.log("\nLocal Team Sample Player:");
                console.log(JSON.stringify(detail.equipo_local[0], null, 2));
                // Check unique categories
                const localCats = [...new Set(detail.equipo_local.map(p => p.categoria || p.category || p.cat || 'N/A'))];
                console.log("Unique Local Player Categories (guessing names):", localCats);
            }
