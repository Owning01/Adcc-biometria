...
if ($gradleResult -eq 0 -and (Test-Path "android/app/build/outputs/apk/debug/app-debug.apk")) {
    ...
    Write-Host " [OK] APK actualizado en carpeta build_outputs/" -ForegroundColor Green
}
else {
...
if ($LASTEXITCODE -ne 0) { 
    Write-Host " [ERROR] Error fatal: No se pudo subir el Hosting." -ForegroundColor Red
    exit 1 
}
